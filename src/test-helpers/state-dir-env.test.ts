...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    opencrab: process.env.OPENCRAB_STATE_DIR,
    legacy: process.env.OPENCLAW_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.OPENCRAB_STATE_DIR).toBe(snapshot.opencrab);
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips OPENCRAB_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/opencrab-state-dir-test");
    expect(process.env.OPENCRAB_STATE_DIR).toBe("/tmp/opencrab-state-dir-test");
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("opencrab-state-dir-env-", async ({ tempRoot, stateDir }) => {
      capturedTempRoot = tempRoot;
      capturedStateDir = stateDir;
      expect(process.env.OPENCRAB_STATE_DIR).toBe(stateDir);
      expect(process.env.OPENCLAW_STATE_DIR).toBeUndefined();
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("opencrab-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
  ...
  it("withStateDirEnv restores both env vars when legacy var was previously set", async () => {
    ...
    process.env.OPENCRAB_STATE_DIR = "/tmp/original-opencrab";
