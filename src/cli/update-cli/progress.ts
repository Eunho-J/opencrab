...
const STEP_LABELS: Record<string, string> = {
  ...
  "opencrab doctor entry": "Checking doctor entrypoint",
  "opencrab doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g opencrab@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g opencrab@latest --omit=optional");
  }
