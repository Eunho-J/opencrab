...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# OpenCrab Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(opencrab completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# OpenCrab Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(opencrab completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "opencrab") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c opencrab -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'opencrab gateway' and not 'opencrab other gateway', we need to check the sequence.
