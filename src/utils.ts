...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".opencrab");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.OPENCRAB_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$OPENCRAB_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via OPENCRAB_STATE_DIR.
