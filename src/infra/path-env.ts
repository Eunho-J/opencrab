...
function candidateBinDirs(opts: EnsureOpenClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `opencrab` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "opencrab");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.OPENCRAB_ALLOW_PROJECT_LOCAL_BIN) ||
    isTruthyEnvValue(process.env.OPENCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "opencrab"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `opencrab` CLI can run
