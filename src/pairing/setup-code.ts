...
import type { OpenCrabConfig } from "../config/types.js";
...
function resolveGatewayPort(cfg: OpenCrabConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: OpenCrabConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  const mode = cfg.gateway?.auth?.mode;
  const token = env.OPENCRAB_GATEWAY_TOKEN?.trim() || cfg.gateway?.auth?.token?.trim();
  const password = env.OPENCRAB_GATEWAY_PASSWORD?.trim() || cfg.gateway?.auth?.password?.trim();
