...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.opencrabCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.opencrabCanvasA2UIAction &&
        typeof window.opencrabCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.opencrabSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("opencrab:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android OpenCrab node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.opencrabSendUserAction === "function"
        ? window.opencrabSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.OPENCRAB_SKIP_CANVAS_HOST)) {
    return true;
  }
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>OpenCrab Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
