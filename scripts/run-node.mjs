...
const logRunner = (message, deps) => {
  if (deps.env.OPENCRAB_RUNNER_LOG === "0" || deps.env.OPENCLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[opencrab] ${message}\n`);
};

const runOpenCrab = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["opencrab.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
