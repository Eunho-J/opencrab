---
summary: "CLI reference for `opencrab onboard` (interactive onboarding wizard)"
read_when:
...
```bash
opencrab onboard
opencrab onboard --flow quickstart
opencrab onboard --flow manual
opencrab onboard --mode remote --remote-url ws://gateway-host:18789
```

Non-interactive custom provider:

```bash
opencrab onboard --non-interactive \
...
- Fastest first chat: `opencrab dashboard` (Control UI, no channel setup).
